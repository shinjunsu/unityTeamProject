...
namespace KWNET.Message
{
    ...
    public class StreamBinMessageSerializer : Serializer<StreamBinMessageHeader>
    {
		...
		protected override void _Serialize<T>(int messageID, T managedData)
        {
			...
			int total_size = SBPacket.GetSize_Data() + SerializedHeaderSize;
			SBPacket.m_Head.WriteBin(bw);
			bw.Write( total_size );
			bw.Write( SBPacket.GetBuff() , 0 , total_size - SerializedHeaderSize );
    ...
    public class StreamBinMessageDeserializer : Deserializer<StreamBinMessageHeader>
    {
		...
		// 전체 사이즈 체크 추가..
           if (tcpSession.RecvCircularBuffer.ReadableSize < total_size )
			...
			SBRecvBuffer buff_obj =	SBReceiver.GetRecvBuffObj();
	...
	public	class SBReceiver
	{
		...
		public  Queue<SBRecvBuffer>	qRecvBuff = new Queue<SBRecvBuffer>();
		...
		public	void init()
		{
			...
			{
				SBRecvBuffer obj =	new SBRecvBuffer();
				obj.Init();
		...
		static	public SBRecvBuffer	GetRecvBuffObj()
		{
		...
		static	public	void	ReturnRecvBuffObj( SBRecvBuffer buff_obj )
		{
